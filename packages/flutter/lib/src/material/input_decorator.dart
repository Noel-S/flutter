...
class _RenderDecoration extends RenderBox with SlottedContainerRenderObjectMixin<_DecorationSlot, RenderBox> {
  ...
  _RenderDecorationLayout _layout(BoxConstraints layoutConstraints) {
    ...
    final double topInputBaseline = contentPadding.top + topHeight + inputInternalBaseline + baselineAdjustment + interactiveAdjustment + densityOffset.dy / 2.0;
  ...
  double computeMinIntrinsicWidth(double height) {
    return _minWidth(icon, height) + contentPadding.left + _minWidth(prefixIcon, height) + _minWidth(prefix, height) + math.max(_minWidth(input, height), _minWidth(hint, height)) + _minWidth(suffix, height) + _minWidth(suffixIcon, height) + contentPadding.right;
  }

  @override
  double computeMaxIntrinsicWidth(double height) {
    return _maxWidth(icon, height) + contentPadding.left + _maxWidth(prefixIcon, height) + _maxWidth(prefix, height) + math.max(_maxWidth(input, height), _maxWidth(hint, height)) + _maxWidth(suffix, height) + _maxWidth(suffixIcon, height) + contentPadding.right;
  }
    ...
    if (label != null) {
      ...
      switch (textDirection) {
        case TextDirection.rtl:
          ...
          decoration.borderGap.start = lerpDouble(labelX + _boxSize(label).width + offsetToPrefixIcon, _boxSize(container).width / 2.0 + floatWidth / 2.0, floatAlign);
        ...
        case TextDirection.ltr:
          ...
          decoration.borderGap.start = lerpDouble(labelX - _boxSize(icon).width + offsetToPrefixIcon, _boxSize(container).width / 2.0 - floatWidth / 2.0, floatAlign);
      }
...
class _InputDecoratorState extends State<InputDecorator> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    final bool labelIsInitiallyFloating = widget.decoration.floatingLabelBehavior == FloatingLabelBehavior.always || (widget.decoration.floatingLabelBehavior != FloatingLabelBehavior.never && widget._labelShouldWithdraw);
  ...
  Color _getIconColor(ThemeData themeData, InputDecorationTheme defaults) {
    return MaterialStateProperty.resolveAs(decoration.iconColor, materialState) ?? MaterialStateProperty.resolveAs(themeData.inputDecorationTheme.iconColor, materialState) ?? MaterialStateProperty.resolveAs(defaults.iconColor!, materialState);
  }

  Color _getPrefixIconColor(ThemeData themeData, InputDecorationTheme defaults) {
    return MaterialStateProperty.resolveAs(decoration.prefixIconColor, materialState) ?? MaterialStateProperty.resolveAs(themeData.inputDecorationTheme.prefixIconColor, materialState) ?? MaterialStateProperty.resolveAs(defaults.prefixIconColor!, materialState);
  }

  Color _getSuffixIconColor(ThemeData themeData, InputDecorationTheme defaults) {
    return MaterialStateProperty.resolveAs(decoration.suffixIconColor, materialState) ?? MaterialStateProperty.resolveAs(themeData.inputDecorationTheme.suffixIconColor, materialState) ?? MaterialStateProperty.resolveAs(defaults.suffixIconColor!, materialState);
  }
  ...
  bool get _hasInlineLabel {
    return !widget._labelShouldWithdraw && (decoration.labelText != null || decoration.label != null) && decoration.floatingLabelBehavior != FloatingLabelBehavior.always;
  }
  ...
  TextStyle _getInlineLabelStyle(ThemeData themeData, InputDecorationTheme defaults) {
    ...
    final TextStyle? style = MaterialStateProperty.resolveAs(decoration.labelStyle, materialState) ?? MaterialStateProperty.resolveAs(themeData.inputDecorationTheme.labelStyle, materialState);
  ...
  TextStyle _getInlineHintStyle(ThemeData themeData, InputDecorationTheme defaults) {
    ...
    final TextStyle? style = MaterialStateProperty.resolveAs(decoration.hintStyle, materialState) ?? MaterialStateProperty.resolveAs(themeData.inputDecorationTheme.hintStyle, materialState);
  ...
  TextStyle _getFloatingLabelStyle(ThemeData themeData, InputDecorationTheme defaults) {
    TextStyle defaultTextStyle = MaterialStateProperty.resolveAs(isFocused ? defaults.focusedFloatingLabelStyle! : defaults.floatingLabelStyle!, materialState);
    ...
    defaultTextStyle = defaultTextStyle.merge((isFocused ? decoration.focusedFloatingLabelStyle : decoration.floatingLabelStyle) ?? decoration.labelStyle);

    final TextStyle? style = MaterialStateProperty.resolveAs((isFocused ? decoration.focusedFloatingLabelStyle : decoration.floatingLabelStyle), materialState) ??
        MaterialStateProperty.resolveAs((isFocused ? themeData.inputDecorationTheme.focusedFloatingLabelStyle : themeData.inputDecorationTheme.floatingLabelStyle), materialState);
  ...
  TextStyle _getHelperStyle(ThemeData themeData, InputDecorationTheme defaults) {
    return MaterialStateProperty.resolveAs(defaults.helperStyle!, materialState).merge(MaterialStateProperty.resolveAs(decoration.helperStyle, materialState));
  }
    ...
    if (themeData.useMaterial3) {
      ...
      return border.copyWith(
        borderSide: BorderSide(
          ...
          width: ((decoration.isCollapsed ?? themeData.inputDecorationTheme.isCollapsed) || decoration.border == InputBorder.none || !decoration.enabled)
              ? 0.0
              : isFocused
                  ? 2.0
                  : 1.0,
  ...
  Widget build(BuildContext context) {
    ...
    final InputDecorationTheme defaults = Theme.of(context).useMaterial3 ? _InputDecoratorDefaultsM3(context) : _InputDecoratorDefaultsM2(context);
    ...
    if (decoration.isCollapsed ?? themeData.inputDecorationTheme.isCollapsed) {
      ...
      if (decoration.filled ?? false) {
        contentPadding = decorationContentPadding ?? (decorationIsDense ? const EdgeInsets.fromLTRB(12.0, 8.0, 12.0, 8.0) : const EdgeInsets.fromLTRB(12.0, 12.0, 12.0, 12.0));
      } else {
        // Not left or right padding for underline borders that aren't filled
        // is a small concession to backwards compatibility. This eliminates
        // the most noticeable layout change introduced by #13734.
        contentPadding = decorationContentPadding ?? (decorationIsDense ? const EdgeInsets.fromLTRB(0.0, 8.0, 0.0, 8.0) : const EdgeInsets.fromLTRB(0.0, 12.0, 0.0, 12.0));
      }
    } else {
      floatingLabelHeight = 0.0;
      contentPadding = decorationContentPadding ?? (decorationIsDense ? const EdgeInsets.fromLTRB(12.0, 20.0, 12.0, 12.0) : const EdgeInsets.fromLTRB(12.0, 24.0, 12.0, 16.0));
    }
...
class InputDecoration {
  ...
  const InputDecoration({
    ...
    this.focusedFloatingLabelStyle,
    this.helperText,
    this.helperStyle,
    this.helperMaxLines,
    this.hintText,
  ...
  const InputDecoration.collapsed({
    ...
    this.enabled = true,
        ...
        focusedFloatingLabelStyle = null,
        helperText = null,
        helperStyle = null,
        helperMaxLines = null,
        hintMaxLines = null,
  ...
  /// The style to use for [InputDecoration.labelText] when the label is
  /// above (i.e., vertically adjacent to) the input field and the state its focused.
  final TextStyle? focusedFloatingLabelStyle;
  ...
  InputDecoration copyWith({
    ...
    TextStyle? focusedFloatingLabelStyle,
    String? helperText,
    ...
    return InputDecoration(
      ...
      focusedFloatingLabelStyle: focusedFloatingLabelStyle ?? this.focusedFloatingLabelStyle,
  ...
  InputDecoration applyDefaults(InputDecorationTheme theme) {
    return copyWith(
      ...
      focusedFloatingLabelStyle: focusedFloatingLabelStyle ?? theme.focusedFloatingLabelStyle,
  ...
  bool operator ==(Object other) {
    ...
    return other is InputDecoration &&
        ...
        other.focusedFloatingLabelStyle == focusedFloatingLabelStyle &&
  ...
  String toString() {
    final List<String> description = <String>[
      ...
      if (focusedFloatingLabelStyle != null) 'focusedFloatingLabelStyle: "$focusedFloatingLabelStyle"',
...
class InputDecorationTheme with Diagnosticable {
  ...
  /// The style to use for [InputDecoration.labelText] when the label is above (i.e., vertically adjacent to) the input field and with a focused state.
  final TextStyle? focusedFloatingLabelStyle;
  ...
  InputDecorationTheme merge(InputDecorationTheme? inputDecorationTheme) {
    ...
    return copyWith(
      ...
      focusedFloatingLabelStyle: focusedFloatingLabelStyle ?? inputDecorationTheme.focusedFloatingLabelStyle,
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(DiagnosticsProperty<TextStyle>('focusedFloatingLabelStyle', focusedFloatingLabelStyle, defaultValue: defaultTheme.focusedFloatingLabelStyle));
...
class _InputDecoratorDefaultsM2 extends InputDecorationTheme {
  ...
  @override
  TextStyle? get focusedFloatingLabelStyle => MaterialStateTextStyle.resolveWith((Set<MaterialState> states) {
        if (states.contains(MaterialState.disabled)) {
          return TextStyle(color: Theme.of(context).disabledColor);
        }
        if (states.contains(MaterialState.error)) {
